...
This was expected. Earlier in the lab guide, you were provided with calculations the context length for the Granite 8B model. Let's revisit that information.
...
Red Hat AI Inference 3.4 introduces *Automatic context sizing*, that smooths out deployment friction. It prevents frustrating Out-of-Memory crashes during launches and ensures that the server automatically maximizes 100% of the available VRAM without manual guesswork.
...
. In the terminal where RHAII is running, press `Ctrl+C`.
