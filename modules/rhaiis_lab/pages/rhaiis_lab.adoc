...
++++
<iframe 
  src="https://demo.arcade.software/ZROoS9bYRFT80X8CIeNL?embed&embed_mobile=inline&embed_desktop=inline&show_copy_link=true"
  width="100%" 
  height="600px" 
  frameborder="0" 
  allowfullscreen
...
. Stop the running RHAII container by pressing `Ctrl+C` in its terminal.
...
++++
<iframe 
  src="https://demo.arcade.software/Hisn83KTuYuEmc3rNqgn?embed&embed_mobile=inline&embed_desktop=inline&show_copy_link=true"
  width="100%" 
  height="600px" 
  frameborder="0" 
  allowfullscreen
...
An NVIDIA A10G GPU has 24 GB of vRAM. Given that the *RedHatAI/granite-3.1-8b-instruct* model requires ~17.0 Gi for its weights and system overhead, what is the maximum context length (in tokens) you could configure for the KV Cache to ensure the entire workload fits on the device (assuming 0.15625 MB per token and 95% GPU usage)?
****
...
++++
<iframe 
  src="https://demo.arcade.software/9XxjzjtpF895ejzDrlb5?embed&embed_mobile=inline&embed_desktop=inline&show_copy_link=true"
  width="100%" 
  height="600px" 
  frameborder="0" 
  allowfullscreen
